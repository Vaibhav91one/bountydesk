---
name: bountydesk-security-audit
description: Vulnerability-class reference and finding discipline from Cloudflare's security-audit skill. Use in GUIDANCE MODE ONLY when triaging a single reported vulnerability, to classify it, name the affected principal and trust boundary, judge a plausible exploit path, and write a finding with evidence and the smallest effective fix. Load only the attack-class file that fits the report. Never run the six-phase full audit, create an output directory, or write audit artifacts here.
---
