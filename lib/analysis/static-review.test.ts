...
test("an unreachable GitHub yields an empty corpus and a text-only static review, not an error", async () => {
  ...
  try {
    const source = await gatherStaticSource({ repoFullName: "owner/repo", ref: null, reportText: "search" });
...
test("a cancelled run is not swallowed as an empty corpus", async () => {
  ...
  await assert.rejects(
    gatherStaticSource({ repoFullName: "owner/repo", ref: null, reportText: "x" }, { signal: controller.signal }),
    /shutting down/,
  );
});
