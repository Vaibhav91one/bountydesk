...
/** The distinct `owner/repo` names a text links to, lowercased, in order of first mention. */
export function repositoryMentions(text: string): string[] {
  ...
  for (const match of text.slice(0, MAX_BODY).matchAll(LINK)) {
    ...
    found.add(`${owner}/${repo}`.toLowerCase());
...
export async function suggestTargets(body: string): Promise<TargetSuggestion> {
  ...
  const rows = await db
    ...
    .where(
      and(
        inArray(sql`lower(${connectedRepository.fullName})`, mentions),
        isNull(targetProfile.retiredAt),
      ),
    );

  const live = rows.filter(grantIsLive);
  const matched = mentions.flatMap((name) =>
    live
      .filter((row) => row.fullName.toLowerCase() === name)
      .map(({ profileId, profileName, fullName }) => ({ profileId, profileName, fullName })),
  );
  const unconnected = mentions.filter((name) => !matched.some((m) => m.fullName.toLowerCase() === name));
  return { matched, unconnected };
}
