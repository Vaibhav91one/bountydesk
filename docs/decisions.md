...
- **Qodo review was mandatory for hackathon eligibility.** Live rules at the time: every substantive change went through a GitHub PR reviewed by Qodo before merge; direct pushes to main did not count; README needed the exact heading `## Qodo Code Review Evidence`. The window closed and the Qodo trial expired in September 2026, so the gate was removed; merges since then need a PR, the `build` check, and resolved conversations, and the Qodo trail in `README.md` is history. Source: https://www.wemakedevs.org/hackathons/trueforge/rules
