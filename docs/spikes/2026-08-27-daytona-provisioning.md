...
| `http://1.1.1.1` (no DNS involved) | **403** `Internet is restricted on Tier 1 and Tier 2.` |
...
Only two outcomes count as blocked: nothing came back at all, or what came back is the
interception proxy, matched on both its 403 and its denial body. Status alone cannot tell
refusal from a reachable service saying no, since a metadata endpoint that wants a header
answers 4xx too, so a 4xx without the denial signature counts as reached. The run also checks
that `curl` exists before trusting any of this: a missing binary would make every probe "fail
to connect" and hand back a clean bill of health.

The literal-IP probe is plain HTTP on purpose. Over TLS it failed with a connection reset,
which reads as the network refusing the request but is really a certificate that does not match
a bare IP. On HTTP the same address returns the proxy's 403, so what was recorded as blocked
egress is now recorded as blocked egress for the actual reason.
